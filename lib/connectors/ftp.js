...
class ConnectorFTP extends Connector {
  ...
  _getFileTo(path, targetPath, completed, progress) {
    const npath = Path.posix.resolve(path);
  ...
  _getFolderTo(remotePath, targetPath, recursive, completed, progress) {
    const npath = Path.posix.resolve(remotePath);
    ...
    this.list(npath, recursive, (lError, list) => {
      ...
      const n = () => {
        ...
        const nLocal = Path.join(targetPath, '..', item.name);
  ...
  getTo(remotePath, targetPath, recursive, completed, progress) {
    if (!this._isConnectedApply(completed)) return;

    this.type(remotePath, (type) => {
      if (type === 'f') {
        this._getFileTo(remotePath, targetPath, completed, progress);
      } else {
        this._getFolderTo(remotePath, targetPath, recursive, completed, progress);
      }
    });
  }
  ...
  putTo(sourcePath, targetPath, completed, progress) {
    ...
    const remote = this.client.toRemote(targetPath);
    ...
    if (FS.isFileSync(sourcePath)) {
      ...
      return this.ftp.put(sourcePath, remote, (err) => {
        let fatal = false;

        if (/Permission denied/.test(err)) {
          isPermissionDenied(sourcePath);
          fatal = true;
          return e(err);
        }
        ...
        if (err && !fatal) {
          this.mkdir(Path.dirname(remote)
            .replace(/\\/g, '/'), true, () => {
              this.ftp.put(sourcePath, remote, (putError) => {
                if (pool) clearInterval(pool);
                return e(putError);
              });
            });
    ...
    return traverseTree(sourcePath, (list) => {
      this.mkdir(remote, true, () => {
        ...
        const n = () => {
          ...
          const nRemote = Path.posix.join(Path.dirname(remote), this.client.toRemote(item.name));

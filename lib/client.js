...
export default class Client {
  ...
  setProjectName() {
    ...
    const rootNameEl = document.querySelector('.ftptree-view .project-root > .header span');
    if (rootNameEl) rootNameEl.textContent = rootName;
  }
  ...
  getProjectPath() {
    ...
    if (multipleHostsEnabled() === true) {
      const currentProject = document.querySelector('.tree-view .project-root');
      if (currentProject) {
        const nameSpan = currentProject.querySelector('.header span.name');
        projectPath = nameSpan ? nameSpan.getAttribute('data-path') : null;
      }
    } else {

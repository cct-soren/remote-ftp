  ...
  const commands = {
    ...
    'remote-ftp:download-selected-local': {
      ...
      command() {
        ...
        const treeSelected = Array.from(document.querySelectorAll('.tree-view .selected'));
        const requestedTransfers = treeSelected.length;

        let successfulTransfers = 0;
        let attemptedTransfers = 0;

        treeSelected.forEach((elem) => {
    ...
    'remote-ftp:upload-selected': {
      ...
      command() {
        ...
        const locals = Array.from(document.querySelectorAll('.tree-view .selected')).map(el => (el.getPath ? el.getPath() : ''));
    ...
    'remote-ftp:sync-with-local': {
      ...
      command() {
        ...
        const locals = Array.from(document.querySelectorAll('.tree-view .selected')).map(checkPaths);
    ...
    'remote-ftp:permission-selected': {
      ...
      command() {
        ...
        const isRoot = remotes[0].element.classList.contains('project-root');
        if (isRoot) return;
